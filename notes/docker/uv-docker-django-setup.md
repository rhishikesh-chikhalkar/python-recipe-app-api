...
### 5. Application Directory Layout & Industry Rationale (`app/app/`)

In containerized Python/Django applications, source code is organized inside an `app/` subfolder containing a nested `app/app/` configuration module:

- **Outer `app/` Directory:** The source code root containing `manage.py` and application modules (`core`, `user`, `recipe`).
- **Inner `app/` Package:** The Django project configuration package (`settings.py`, `urls.py`, `wsgi.py`) generated by `django-admin startproject app .`.

#### Industry Best Practices & Engineering Rationale:

1. **Separation of Repository Infrastructure & Source Code:**
   Keeps root-level tooling files (`Dockerfile`, `docker-compose.yml`, `pyproject.toml`, `.github/`, `README.md`) separate from executable Python source code.
2. **Docker Volume Isolation:**
   In development (`volumes: - ./app:/app`), mounting *only* the `app/` directory into `/app` inside Docker ensures live hot-reloading affects source code only. It prevents host OS artifacts (like host `.venv`, `.git`, host OS cache files) from overwriting or corrupting the container's isolated `/py` virtual environment.
3. **Python Import Namespace Safety (`src` / `app` Layout Standard):**
   Encapsulating source code in a subfolder prevents accidental root-level implicit imports during local testing, ensuring import resolution behaves identically across local and containerized environments.
4. **Least-Privilege Security Boundaries:**
   Combining `WORKDIR /app` with `USER django-user` restricts the running application process to its own working directory (`/app`), denying write access to host configurations or system root paths.
5. **Monorepo / Multi-Service Scalability:**
   If the repository expands to include auxiliary services (e.g., `frontend/`, `infra/`), having source code encapsulated in `app/` avoids costly repository refactoring.
...
### Q7: Why do production Python/Django repositories place source code inside an `app/` directory and mount it to `/app` inside Docker?

**Answer:**
Organizing source code in an `app/` subfolder and mounting it to `/app` in Docker is an industry standard pattern for five key reasons:

1. **Infrastructure vs. Code Separation:** Keeps repository tooling files (`Dockerfile`, `docker-compose.yml`, `pyproject.toml`, `.github/`) isolated from application source code.
2. **Docker Volume Isolation:** In development (`volumes: - ./app:/app`), mounting only `app/` ensures host OS artifacts (host `.venv`, `.git`, temporary files) do not leak into or corrupt the container's isolated `/py` environment.
3. **Import Namespace Protection (`src`/`app` layout standard):** Prevents accidental root-level implicit imports during local testing, ensuring consistent package resolution between local and container environments.
4. **Least-Privilege Container Security:** Pairing `WORKDIR /app` with `USER django-user` confines process privileges to `/app`, preventing unauthorized modifications to root system files.
5. **Monorepo Readiness:** Simplifies adding sibling folders (e.g., `frontend/`, `infra/`) in the future without refactoring existing application code.
